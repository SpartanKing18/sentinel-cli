...
const { priceOf } = require("../nexus/pricing"); // per-model $ table, for downshift math
...
// A model can often be safely "downshifted" to a cheaper tier for work that
// doesn't need the top model. from = current-model matcher; price = the cheaper
// tier's $/M. Only applied when the cheaper tier's output price is actually lower.
const DOWNSHIFTS = [
  { from: /opus/i, to: "sonnet", price: { in: 3, out: 15 } },
  { from: /sonnet/i, to: "haiku", price: { in: 0.8, out: 4 } },
  { from: /gemini(?!.*flash)/i, to: "flash", price: { in: 0.3, out: 2.5 } },
];

// analyzeSavings(records, { mechanicalFraction }) -> pure cost-savings analysis.
// Two independent levers, both quantified from the non-sensitive ledger:
//   • local routing — a fraction (default 30%) of PAID turns are mechanical
//     (test/build/lint/format) and could run free on local Ollama.
//   • model downshift — the SAME token volume re-priced on a cheaper tier
//     (opus→sonnet, sonnet→haiku, gemini→flash), per model.
// Plus a run-rate and a 30-day projection from active-day spend. Pure & unit-tested.
function analyzeSavings(records, opts) {
  opts = opts || {};
  const localFrac = typeof opts.mechanicalFraction === "number"
    ? Math.max(0, Math.min(1, opts.mechanicalFraction)) : 0.30;
  const models = {}, days = {};
  let totalCost = 0, paidCost = 0, paidTurns = 0, freeTurns = 0;
  for (const r of records) {
    const cost = r.cost || 0; totalCost += cost;
    const free = r.engine === "ollama" || cost === 0;
    if (free) freeTurns++; else { paidTurns++; paidCost += cost; }
    const name = r.model || r.engine || "unknown";
    const m = models[name] || (models[name] = { model: name, engine: r.engine || "", turns: 0, inTok: 0, outTok: 0, cost: 0 });
    m.turns++; m.inTok += r.inTok || 0; m.outTok += r.outTok || 0; m.cost += cost;
    if (typeof r.ts === "string") days[r.ts.slice(0, 10)] = 1;
  }
  const byModel = Object.keys(models).map((name) => {
    const m = models[name], cur = priceOf(name);
    let downshift = null;
    for (const d of DOWNSHIFTS) {
      if (d.from.test(name) && d.price.out < cur.out) {
        const curCost = (m.inTok / 1e6) * cur.in + (m.outTok / 1e6) * cur.out;
        const altCost = (m.inTok / 1e6) * d.price.in + (m.outTok / 1e6) * d.price.out;
        downshift = { to: d.to, altCost, saved: Math.max(0, curCost - altCost) };
        break;
      }
    }
    return Object.assign({}, m, { downshift });
  }).sort((a, b) => b.cost - a.cost);
  const downshiftSaved = byModel.reduce((s, m) => s + (m.downshift ? m.downshift.saved : 0), 0);
  const localSaved = paidCost * localFrac;
  const activeDays = Math.max(1, Object.keys(days).length);
  const dailyAvg = totalCost / activeDays;
  return {
    turns: records.length, totalCost, paidCost, paidTurns, freeTurns,
    activeDays, dailyAvg, projected30: dailyAvg * 30,
    localFrac, localSaved, downshiftSaved, byModel,
    totalSavable: localSaved + downshiftSaved,
  };
}

// renderSavings(analysis, { project }) -> deterministic plain-text (caller colorizes).
function renderSavings(a, opts) {
  opts = opts || {};
  const L = [];
  L.push("Nexus cost-savings analysis" + (opts.project ? " — " + opts.project : ""));
  L.push("  Spend       " + MONEY(a.totalCost) + " over " + a.turns + " turn" + (a.turns === 1 ? "" : "s") +
    "  (" + a.paidTurns + " paid · " + a.freeTurns + " free/local)");
  L.push("  Run rate    " + MONEY(a.dailyAvg) + "/day  →  ~" + MONEY(a.projected30) + "/30d" +
    "  (" + a.activeDays + " active day" + (a.activeDays === 1 ? "" : "s") + ")");
  L.push("");
  L.push("  Opportunities");
  L.push("    Local routing   ~" + MONEY(a.localSaved) + "   move ~" + Math.round(a.localFrac * 100) +
    "% mechanical paid turns to free local Ollama (/cowork · -e ollama)");
  if (a.downshiftSaved > 0)
    L.push("    Model downshift ~" + MONEY(a.downshiftSaved) + "   run fitting work on a cheaper tier (see below)");
  const ds = a.byModel.filter((m) => m.downshift && m.downshift.saved > 0);
  if (ds.length) {
    L.push(""); L.push("  Downshift by model (same token volume, cheaper tier)");
    for (const m of ds) L.push("    " + m.model.padEnd(26) + MONEY(m.cost).padEnd(11) + "→ " +
      m.downshift.to.padEnd(7) + " ≈ " + MONEY(m.downshift.altCost) + "   save " + MONEY(m.downshift.saved));
  }
  L.push("");
  L.push("  Savings available ~" + MONEY(a.totalSavable) + grayNote());
  return L.join("\n");
}
function grayNote() { return "   (illustrative — apply the levers that fit your work)"; }

module.exports = { appendUsage, loadUsage, summarize, renderReport, analyzeSavings, renderSavings, MONEY, TOK };
