...
// Does an IP fall within a CIDR? true / false, or null if either input is invalid.
function inCidr(ip, cidr) {
  const ipn = ipToInt(ip), c = cidrCalc(cidr);
  if (ipn === null || !c) return null;
  return ipn >= ipToInt(c.network) && ipn <= ipToInt(c.broadcast);
}
module.exports = { TOP_PORTS, parsePorts, idHash, parseCve, cidrCalc, ipToInt, inCidr };
