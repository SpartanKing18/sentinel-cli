...
async function cli(args) {
  ...
  if (CMD_MAP[cmd]) { console.log(CMD_MAP[cmd].run({ rest, c: { red, green, yellow, cyan, gray, bold } })); }
  ...
  else if (cmd === "stats" || cmd === "loc") {
    // Headless codebase overview — files, lines & languages, largest files.
    const s = summarizeStats(scanStats(process.cwd()));
    if (rest.includes("--json")) { console.log(JSON.stringify(s, null, 2)); }
    else if (!s.totalFiles) { console.log("  " + gray("no recognized source files in this directory")); }
    else { const kb = (n) => n >= 1e6 ? (n / 1e6).toFixed(1) + " MB" : (n / 1e3).toFixed(0) + " KB"; const langs = rankedLangs(s); const w = Math.max.apply(null, langs.map((l) => l[0].length));
    ...
    const args = rest.filter((a, i) => !a.startsWith("--") && !(mi >= 0 && i === mi + 1));
    const tag = (args[0] || "").toUpperCase();
    const { items, files } = scanTodos(process.cwd());
    const list = tag ? items.filter((it) => it.tag === tag) : items;
    if (rest.includes("--json")) { console.log(JSON.stringify({ summary: summarizeTodos(list), items: rankTodos(list) }, null, 2)); }
    else if (!list.length) { console.log("  " + green("clean") + gray(" — no " + (tag || "TODO/FIXME/HACK") + " markers in " + files + " files")); }
    else { const s = summarizeTodos(list); console.log("  " + bold(String(s.total)) + " marker(s) in " + s.files + " file(s)   " + TODO_TAGS.filter((t) => s.byTag[t]).map((t) => yellow(t) + " " + s.byTag[t]).join(gray(" · "))); rankTodos(list).slice(0, 100).forEach((it) => console.log("  " + cyan(it.file + ":" + it.line) + "  " + yellow(it.tag) + (it.text ? "  " + it.text : ""))); }
    if (!isNaN(max)) { const n = list.length; if (n > max) { console.log(red("  tech-debt gate: " + n + " > " + max + " allowed")); process.exit(1); } }
  }
...
const { appendUsage, loadUsage, summarize, renderReport, analyzeSavings, renderSavings } = require("./lib/governance/usage"); // cost/usage ledger + chargeback report + savings analysis (lib/usage.js)
...
function nexusTui(engine, cwd, nexusMd) {
  return new Promise((resolve) => {
    ...
    const submit = (text) => {
      ...
      // Vision: pull image @mentions out as real attachments (base64) BEFORE @file
      // text-inlining would read them as garbage. Local/API engines get them natively;
      // claude/cli engines get a file-path reference they can Read.
      const imgAttach = [];
      const preText = text.replace(/(^|\s)@(\S+\.(?:png|jpe?g|gif|webp|bmp))\b/gi, (mm, pre, p) => {
        try { const fp = path.resolve(cwd, p); if (fs.statSync(fp).size > 6 * 1024 * 1024) return mm; const ext = p.split(".").pop().toLowerCase(); imgAttach.push({ b64: fs.readFileSync(fp).toString("base64"), mime: "image/" + (ext === "jpg" ? "jpeg" : ext), name: p }); return pre + "[attached image: " + p + "]"; } catch (_) { return mm; }
      });
    ...
    const openPr = () => {
      let branch = "", diff = "", base = "main";
      try { branch = _cp.execSync("git rev-parse --abbrev-ref HEAD", { cwd, encoding: "utf8" }).trim(); } catch (_) { transcript.push({ role: "system", text: "/pr needs a git repo" }); render(); return; }
      try { base = (_cp.execSync("git remote show origin 2>/dev/null | sed -n 's/.*HEAD branch: //p'", { cwd, encoding: "utf8" }).trim()) || "main"; } catch (_) {}
      try { diff = _cp.execSync("git -c color.ui=never diff " + JSON.stringify(base) + "...HEAD", { cwd, encoding: "utf8" }); } catch (_) {}
      if (!diff.trim()) { try { diff = _cp.execSync("git -c color.ui=never diff HEAD", { cwd, encoding: "utf8" }); } catch (_) {} }
      if (!diff.trim()) { transcript.push({ role: "system", text: "no changes to open a PR for — commit or make changes first" }); render(); return; }
      transcript.push({ role: "user", text: "/pr" });
      const block = { role: "nexus", items: [] }; transcript.push(block);
      scroll = 0; busy = true; busyStart = Date.now(); busyWord = "Composing"; ctl = makeCtl();
      ...
      (async () => {
        ...
        if (hasBin("gh")) {
          try { out = _cp.execSync("gh pr create --title " + JSON.stringify(title) + " --body " + JSON.stringify(body || title) + " 2>&1", { cwd, encoding: "utf8" }).trim(); opened = /https?:\/\//.test(out); }
          catch (e) { out = ((e.stdout || "") + (e.stderr || "") || e.message || "").toString().trim(); }
        }
        ensureText().full = opened
          ? "pull request opened:\n" + out
          : (hasBin("gh") ? "gh couldn't open the PR (" + out.slice(0, 200) + ") — title & body below:\n\n" : "gh CLI not installed — PR title & body (create it manually):\n\n") + title + "\n\n" + body;
        busy = false; ctl = null; try { saveSession(); } catch (_) {} render();
      })();
    };
    // ---- slash commands ----
    ...
    const handleSlash = (t) => {
      ...
      else if (cmd === "/stats") { const s = summarizeStats(scanStats(cwd)); if (!s.totalFiles) { transcript.push({ role: "system", text: "no recognized source files found here" }); } else { const kb = (n) => n >= 1e6 ? (n / 1e6).toFixed(1) + " MB" : (n / 1e3).toFixed(0) + " KB"; const langs = rankedLangs(s); const w = Math.max.apply(null, langs.map((l) => l[0].length)); const rows = langs.slice(0, 12).map(([k, v]) => "  " + cyan(k.padEnd(w)) + "  " + String(v.files).padStart(4) + gray(" files ") + String(v.lines).padStart(7) + gray(" lines")); const big = s.largest.slice(0, 6).map((f) => "  " + gray(String(f.lines).padStart(6) + "  ") + f.file); transcript.push({ role: "system", text: bold("codebase") + gray("  " + s.totalFiles + " files · " + s.totalLines.toLocaleString() + " lines · " + kb(s.totalBytes) + " · " + langs.length + " languages") + "\n" + rows.join("\n") + "\n\n" + bold("largest files") + "\n" + big.join("\n") }); } }
      else if (cmd === "/env") { const { used, files: sf } = scanEnvTree(cwd); const { declared, files: ef } = readEnvFiles(cwd); const a = auditEnv({ used, declared }); const lines = [bold("env vars") + gray("  " + used.length + " referenced in code · " + declared.length + " documented" + (ef.length ? " (" + ef.join(", ") + ")" : " (no .env.example)"))]; if (a.undocumented.length) lines.push(red("  undocumented (" + a.undocumented.length + ")") + gray(" — used in code, missing from .env.example:") + "\n    " + a.undocumented.join(", ")); if (a.unused.length) lines.push(yellow("  unused (" + a.unused.length + ")") + gray(" — in a .env template but never read:") + "\n    " + a.unused.join(", ")); if (!a.undocumented.length && !a.unused.length) lines.push(green("  clean") + gray(" — code and .env templates agree (common runtime vars ignored)")); transcript.push({ role: "system", text: lines.join("\n") }); }
      else if (cmd === "/deps") { let pkg = null; try { pkg = JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf8")); } catch (_) {} if (!pkg) { transcript.push({ role: "system", text: "no package.json here — /deps audits Node/JS dependency hygiene" }); } else { const a = auditDeps({ pkg, specifiers: scanImports(cwd) }); const lines = [bold("dependencies") + gray("  " + a.declared.length + " declared · " + a.used.length + " imported")]; if (a.missing.length) lines.push(red("  missing (" + a.missing.length + ")") + gray(" — imported but NOT in package.json:") + "\n    " + a.missing.join(", ")); if (a.unused.length) lines.push(yellow("  unused (" + a.unused.length + ")") + gray(" — declared but never imported (may be CLI/config-only):") + "\n    " + a.unused.join(", ")); if (!a.missing.length && !a.unused.length) lines.push(green("  clean") + gray(" — declared and imported sets match")); transcript.push({ role: "system", text: lines.join("\n") }); } }
      else if (cmd === "/todo") { const filter = (arg || "").trim().toUpperCase(); const { items, files } = scanTodos(cwd); const list = filter ? items.filter((it) => it.tag === filter) : items; if (!list.length) { transcript.push({ role: "system", text: filter ? ("no " + filter + " markers found in " + files + " file(s)") : ("no TODO/FIXME/HACK markers in " + files + " scanned file(s) — clean.") }); } else { const s = summarizeTodos(list); const shown = rankTodos(list).slice(0, 60); const head = bold("tech debt") + gray("  " + s.total + " marker" + (s.total === 1 ? "" : "s") + " · " + s.files + " file" + (s.files === 1 ? "" : "s") + "   ") + TODO_TAGS.filter((t) => s.byTag[t]).map((t) => yellow(t) + " " + s.byTag[t]).join(gray(" · ")); const body = shown.map((it) => "  " + cyan(it.file + ":" + it.line) + "  " + yellow(it.tag) + (it.text ? "  " + it.text : "")).join("\n"); transcript.push({ role: "system", text: head + "\n" + body + (s.total > shown.length ? gray("\n  … +" + (s.total - shown.length) + " more") : "") + gray("\n  /todo <TAG> to filter · ask me to fix them") }); } }
      else if (cmd === "/theme") {
