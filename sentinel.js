...
function nexusTui(engine, cwd, nexusMd) {
  return new Promise((resolve) => {
    ...
    const CMDS = [
      ...
      ["/cowork", "strong model codes, weak model does cheap work"], ["/lean", "ask for minimal output (saves output tokens)"], ["/effort", "claude thinking level: low|medium|high"], ["/guard", "preflight destructive commands (enforce|warn|off)"], ["/impact", "session savings (tokens & cost avoided)"], ["/gaps", "list TODO/FIXME markers · /gaps plan"], ["/dream", "consolidate the session into NEXUS.md memory"],
