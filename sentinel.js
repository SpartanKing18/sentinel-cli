...
const { styleNames, styleDirective } = require("./lib/styles"); // output styles (lib/styles.js)
...
function nexusTui(engine, cwd, nexusMd) {
  return new Promise((resolve) => {
    ...
    const handleSlash = (t) => {
      ...
      else if (cmd === "/style") { if (arg && styleNames().includes(arg)) { style = arg; transcript.push({ role: "system", text: "output style: " + bold(arg) + (arg === "default" ? "" : " — " + styleDirective(arg)) + gray("  (applies to every engine)") }); } else transcript.push({ role: "system", text: "output styles (shape HOW the AI works):\n" + styleNames().map((n) => "  " + (n === style ? cyan("● " + n) : "  " + n) + gray(n === "default" ? "" : "  " + styleDirective(n).slice(0, 60))).join("\n") + "\n  usage: /style <name>" }); }
